# Olá, eu sou Leonardo 👋

Desenvolvedor Backend .NET em formação, com foco em APIs REST,arquitetura de dados e FastReport.
Atualmente em transição de Suporte em TI para desenvolvimento backend.

## 🛠️ Tech Stack

![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

## 🚀 Projetos em Destaque

### Gerador Inteligente de Propostas
SaaS MVP fullstack para automação de propostas técnicas e comerciais.
- Backend em C# (.NET 8) com PostgreSQL e autenticação JWT
- Integração com LLMs (Gemini/Grok) para geração de texto via IA
- Exportação para PDF e Word | Frontend em React

### Automação de Vídeos com IA
Pipeline Python para geração automatizada de vídeos com áudio e legendas.
- Integração com Gemini API para geração de roteiro e narração
- Composição de vídeo com MoviePy e legendas sincronizadas

## 📫 Contato

[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:SEU_EMAIL)

---
📍 São Paulo, SP
